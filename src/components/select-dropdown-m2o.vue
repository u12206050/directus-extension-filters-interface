...
function useCurrent() {
	...
	watch(
		...
		(newValue: any) => {
			...
			else if (!currentItem.value && typeof newValue === 'object' && newValue[relatedPrimaryKeyField.value!.field]) {
...
function usePreview() {
	...
	return { displayTemplate, requiredFields };
}
...
async function onPreviewClick() {
	...
	if (newActive === true) return fetchItems();
}
</script>
...
<template>
	...
	<v-menu v-else class="filter-select-m2o" v-model="menuActive" attached :disabled="disabled">
		<template #activator="{ activate, active }">
			<v-skeleton-loader v-if="loadingCurrent" type="input" />
			<div class="input" @click="() => onPreviewClick().then(activate)">
